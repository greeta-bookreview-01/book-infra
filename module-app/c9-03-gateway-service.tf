...
resource "kubernetes_deployment_v1" "gateway_deployment" {
  depends_on = [kubernetes_deployment_v1.gateway_redis_deployment,
                kubernetes_deployment_v1.catalog_deployment,
                kubernetes_deployment_v1.dispatcher_deployment,                
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        container {
          image = "ghcr.io/greeta-bookreview-01/gateway-service:16333b43ac882b27743cfac270e4e9ff4c96f415"
          name  = "gateway"
